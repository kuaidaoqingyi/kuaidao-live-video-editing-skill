# Dedao Live Video Editing Skill
...
这个仓库沉淀的是我们在实际直播剪辑里磨出来的一套流程，不是单个剪辑脚本。
...
- 用 `project.yaml` 记录源文件、目标平台、ASR、版本、QA 状态
...
- 提供 `templates/cut-plan.md` 做剪辑决策记录
...
```bash
git clone https://github.com/kuaidaoqingyi/dedao-live-video-editing-skill.git \
  ~/.codex/skills/dedao-live-video-editing
```
...
```text
按 dedao-live-video-editing 这个 skill 处理这条访谈切片
```
